...
buildingSelect.addEventListener('change', function () {
  ...
  document.getElementById('building-name').textContent = selectedBuilding ? `Building ${selectedBuilding}` : '';
  document.getElementById('floor-name').innerHTML = '';
  document.getElementById('flats-select').innerHTML = '<option value="">Select Flat</option>';
  ...
  if (selectedBuilding) {
    ...
    fetch(excelUrl)
      ...
      .then(buffer => {
        ...
        currentData = XLSX.utils.sheet_to_json(worksheet, { header: ['Floor', 'Flat', 'Type', 'Status', 'Length', 'Width', 'Area', 'Sold'] });
        // Populate floors
...
document.getElementById('floor-select').addEventListener('change', function () {
  ...
  if (selectedFloor) {
    ...
    document.getElementById('floor-svg').data = `/building${selectedBuilding}-floor-${floorMapping[selectedFloor] || selectedFloor}.svg`;
    ...
    const soldCount = flats.filter(f => f.Sold && f.Sold.toLowerCase() === 'yes').length;
    const unsoldCount = totalFlats - soldCount;
    document.getElementById('floor-name').innerHTML = `Floor ${floorMapping[selectedFloor] || selectedFloor}<br>Total Flats: ${totalFlats}, Sold: ${soldCount}, Unsold: ${unsoldCount}`;
...
document.getElementById('flats-select').addEventListener('change', function () {
  ...
  if (highlightedElement) {
    ...
    if (highlightedElement.tagName === 'polygon' || highlightedElement.tagName === 'path') {
